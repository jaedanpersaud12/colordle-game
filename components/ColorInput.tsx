...
export function ColorInput({ colors, onGuess, disabled }: ColorInputProps) {
  ...
  const filteredColors = useMemo(() => {
    ...
    const containsResults = colors.filter((color) =>
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      setSelectedIndex((prev) => prev < 0 ? 0 : Math.min(prev + 1, filteredColors.length - 1));
    ...
    <div className="relative w-full">
      <form onSubmit={handleSubmit} className="flex gap-3">
        <Input
          ...
          autoComplete="off"
        />
        <Button type="submit" disabled={disabled || filteredColors.length === 0}>
          Guess
        </Button>
      </form>
              ...
              {filteredColors.map((color, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="w-14 h-14 border border-border flex-shrink-0"
                      style={{ backgroundColor: color.hex }}
                      title={color.hex}
                    />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold leading-tight">{color.name}</p>
                      <p className={`text-xs font-mono mt-1 ${index === selectedIndex ? 'opacity-70' : 'text-muted-foreground'}`}>
                        {color.hex}
                      </p>
                    </div>
                  </div>
